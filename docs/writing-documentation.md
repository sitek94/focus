...
Audience: people and agents who contribute to this repository. These pages
explain how the repo works today—toolchain, layout, tests, and release—not
end-user product marketing.
...
Describe the system as it works today (“Deploy macOS publishes…”, “The
manifest rejects…”) instead of narrating a rollout (“We now publish…”, “We no
longer accept…”).
...
Put post-call detail in the call result. Anything needed only after a command
runs (full docs index, ranked failures, approval URLs, error bodies) should
come from that command (`make docs-list`, `make release-check`, workflow logs),
not from static instruction text. Static text should not repeat large chunks of
what the next response already shows.

Avoid overlap with generated surfaces. If a host, workflow, or script already
lists schemas, flags, or resources, do not restate those tables verbatim in
docs; link to the workflow, script, or owning page.

## Clarity (Google Technical Writing)
...
- Use bold sparingly—for a single term or warning label, not whole sentences.
